...
s2 = (56767.62-56576.16)*2/1000+(17981.97)/1000
s3 = (3182.38+44700-56767.62+56576.16)/1000
s4 = (3913.68+5625.53)/1000
...
aat.append([0, (s2)*2/vm+aat[-1][1]])
aat.append([aa, 3/20+aat[-1][1]])
aat.append([0, t12-3/20+aat[-1][1]])
aat.append([0, (s3-s12-s20)/vm+aat[-1][1]])
aat.append([-aa, 6/20+aat[-1][1]])
aat.append([0, t20-6/20+aat[-1][1]])

aat.append([aa, 3/20+aat[-1][1]])
aat.append([0, t02+aat[-1][1]-3/20])
aat.append([0, (s4-s20-s02)/vm+aat[-1][1]])
aat.append([-aa, 6/20+aat[-1][1]])
aat.append([0, t20-6/20+aat[-1][1]])
...
def fun1(x):
    ...
    for i in range(len(aat)):
        if(x<aat[i][1]):
            return aat[i][0]
def fun2(x,last,q):
    if((x >= aat[1][1]-q and x <= aat[1][1]+q) or (x >= aat[4][1]-q and x <= aat[4][1]+q) or (x >= aat[7][1]-q and x <= aat[7][1]+q) or (x >= aat[10][1]-q and x <= aat[10][1]+q) or (x >= aat[12][1]-q and x <= aat[12][1]+q) or x >= aat[-1][1]-q):
        return 0
